...
def msg_log(db_words,log_data,sb,nonce,timestamp):
    #设置回复内容
    body_data = '''<xml>
...
def function():
    ...
    if(request.method=="POST"):  
        ...
        if rx_msg_agent!=[]:
            ...
            #返回企业微信及添加日志
            log_data="用户："+staff_real_name+"进入会话"
            return msg_log(db_words,log_data,sb,nonce,timestamp)
    ...
    #             return("故障码",ret)
        ...
        for i in hack:
            if i in str(rx_msg[0]):
                ...
                #返回企业微信及添加日志
                db_words="不要试图让系统错乱哦~"
                log_data="用户"+staff_real_name+"："+rx_msg[0]+"\r\n"+"回复：\r\n"+db_words[0:40]+"\r\n下略"
                return msg_log(db_words,log_data,sb,nonce,timestamp)
